...
public class RestAssuredExercises5Test {
	...
	public void checkFourthSpeedRecordWasSetbyAnAstonMartin() {
		...
		given().
			spec(requestSpec).
		when().get("/xml/speedrecords").
		then().body("speedRecords.car[3].@make",equalTo("Aston Martin"));
	}
	...
	public void checkFourRecordsHaveBeenSetByCarsFromEitherItalyOrGermany() {
		...
		given().
			spec(requestSpec).
		when().get("/xml/speedrecords").
		then().body("speedRecords.car.findAll{it.@country=='Italy'||it.@country=='Germany'}.size()",equalTo(4));
	}
	...
	public void checkTwoRecordsHaveBeenSetByCarsWhoseMakeEndOnBenz() {
		...
		when().get("/xml/speedrecords").
		then().
				body("speedRecords.car.findAll{it.@make=='Benz'||it.@make.contains('Benz')}.size()",equalTo(2));
	}
}